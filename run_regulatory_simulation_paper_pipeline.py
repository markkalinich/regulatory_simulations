...
Output Structure:
    results/REGULATORY_SIMULATION_PAPER/[YYYYMMDD_HHMMSS]/
        ...
        Data/
            ...
            processed_data/              # Metrics, review data, supplementary_tables (S1-S3 CSVs)
        Logs/
            pipeline.log
...
def generate_supplementary_tables_s1_s3(logger: logging.Logger, dry_run: bool = False) -> bool:
    """Write Tables S1-S3 to Data/processed_data/supplementary_tables/."""
    log_subsection(logger, "Supplementary Tables S1-S3")
    script = ROOT / "analysis" / "generate_supplementary_tables.py"
    out = DATA_OUTPUT_DIR / "processed_data" / "supplementary_tables"
    if dry_run:
        logger.info(f"  [DRY RUN] Would run {script.name} → processed_data/supplementary_tables/")
        return True
    out.mkdir(parents=True, exist_ok=True)
    ok = run_python_script(script, ["--output-dir", str(out)], logger, dry_run=False)
    if ok:
        logger.info(f"  ✅ Saved: processed_data/supplementary_tables/ (S1-S3 CSVs)")
    return ok
...
│       ├── supplementary_tables/           # Tables S1-S3 (synthetic data characterization)
...
def run_pipeline(args: argparse.Namespace) -> int:
    ...
    if not generate_supplementary_tables_s1_s3(logger, args.dry_run):
        success = False
