...
The pipeline automatically verifies all numerical claims in the manuscript:

```bash
# Verification runs automatically as final pipeline step
...
- Reads expected claims from `config/manuscript_claims.json`
- Compares against generated data (review statistics, difficult statements, P1/P2 values)
- Reports PASS/FAIL for each claim with actual vs. expected values

**See**: [Manuscript Verification Guide](docs/MANUSCRIPT_VERIFICATION_GUIDE.md) for details on:
- How to update claims when manuscript changes
- Tolerance guidelines for Monte Carlo variations
- MLX vs GGUF model format issues
