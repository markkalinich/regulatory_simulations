...
This repository includes **only the final paper outputs** in `results/REGULATORY_SIMULATION_PAPER/`:
- All main figures (3, 4, 5) and supplementary figures (S3-S10)
- Processed data files and comprehensive metrics
- Model configuration files and manifest

**Intermediate per-task results are excluded** to keep repository size manageable. These can be regenerated by running the pipeline with `--cache-dir cache_v2`.

## Acknowledgments

This codebase was developed extensively using **Cursor AI** (Claude 4.5 Sonnet and Opus) to assist with code generation, refactoring, documentation, and reproducibility infrastructure.
